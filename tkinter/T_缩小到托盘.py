...
class SysTrayIcon(object):
    ...
    def refresh_icon(s):
        ...
        if os.path.isfile(s.icon):
            ...
            hicon = win32gui.LoadImage(hinst, s.icon, win32con.IMAGE_ICON, 0, 0, icon_flags)
        else:  # 找不到图标文件 - 使用默认值
...
class _Main:  # 调用SysTrayIcon的Demo窗口
    ...
    def Hidden_window(s, icon='D:\\1.ico', hover_text="SysTrayIcon.py Demo"):
        ...
        # 24行有自动添加‘退出’，不需要的可删除
        menu_options = (('一级 图标', None, s.switch_icon),
                        ('二级 菜单', None, (('更改 图标', None, s.switch_icon),)))

        s.root.withdraw()  # 隐藏tk窗口
        if s.SysTrayIcon:
            s.SysTrayIcon.update()  # 已经有托盘图标时调用 update 来更新显示托盘图标
        else:
            s.SysTrayIcon = SysTrayIcon(icon,  # 图标
                                        hover_text,  # 光标停留显示文字
                                        menu_options,  # 右键菜单
                                        on_quit=s.exit,  # 退出调用
